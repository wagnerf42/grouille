...
use {CoordinatesHash, HPoint, HashKey, Segment};

/// When two segments overlap (even partially) keep only one of them.
/// Will cut segments into smaller segments in case of partial overlap.
pub fn cut_overlaps(segments: &[Segment]) -> Vec<Segment> {
    let mut angles_hasher = CoordinatesHash::new(0.0001);
    let mut coordinates_hasher = CoordinatesHash::new(0.0001);
    let mut lines: HashMap<(HashKey, HashKey), HashMap<HPoint, isize>> = HashMap::new();
    ...
    for (_, mut aligned_points) in lines.drain() {
        let mut points: Vec<(HPoint, isize)> =
